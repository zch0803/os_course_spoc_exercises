...
```
 ...
 - 回答如下问题：
   ...
   > 124MB，总共内存为128M，其中4MB为FS_SZ,通过查看寄存器信息user=0，堆栈为内核态
   ...
   > 调用write函数时，把1,“S1_2012011384 S2_2012011335”,27这三个参数从右向左压入桟，然后用JSR 0xffffff90（to 0x00000010）跳转到write函数入口，并记录当前PC作为返回地址，返回值放在寄存器a中
   
   分析并说明funcall执行文件的格式和内容
   
   > funcall文件头的部分为hdr数据,然后是text（程序指令），最后是data（数据）
　
